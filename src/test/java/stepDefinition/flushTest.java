...
public class flushTest {
	...
	@Given("^AI hand is one card off from flush and oponent has double$")
	public void ai_hand_is_one_card_off_from_royal_flush() throws Throwable {
		System.out.println("THIS IS THE TEST FOR  FLUSH ONE OFF");
		pokTest.constructCards(pokTest.readFile("assignment2Features.txt").get(4));
		pokTest2.constructCards(pokTest.readFile("flushPermutation.txt").get(0));
		pokTest3.constructCards(pokTest.readFile("flushPermutation.txt").get(1));
		pokTest4.constructCards(pokTest.readFile("flushPermutation.txt").get(2));
		
	}
	
	@When("^AI hand swaps its card for C5 to complete flush$")
	...
	@Then("^AI hand wins with a flush$")
